...
def load_data( user2info,user2traj,):
    ...
    for user in tqdm.tqdm(user2traj):
...
with open('user2info_Ten.json', 'r') as f:
    user2info = json.load(f,)
print('user2info loaded')
with open('user2traj_Ten.json', 'r') as f:
    user2traj = json.load(f,)
print('user2traj loaded')
...
pickle.dump(train_loader,open('./sep_train_test/train.pkl','wb'))
valid_dataset = torch.utils.data.TensorDataset(torch.from_numpy(x[int(0.8*x.shape[0]):int(0.9*x.shape[0])]), torch.from_numpy(y[int(0.8*x.shape[0]):int(0.9*x.shape[0])]))
valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=True)
pickle.dump(valid_loader,open('./sep_train_test/test0.8-0.9.pkl','wb'))

train_loader = pickle.load(open('./sep_train_test/train.pkl','rb'))
...
def Loss_even(input):
    ...
    output = -torch.log(input).mean(-1)
    return output.mean()

valid_loss_best = 1e5

model.load('logs/model_distan_last.pkl')
...
for epoch in range(args.num_epochs):
    ...
    for x,y in valid_loader:
        ...
        loss_even = 0
        for i in range(6):
            loss_even += (Loss_even(dis_item[i]))
        loss = (loss_gender + loss_income + loss_edu + loss_even)*args.converge_coeff + loss_traj 
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    for x,y in valid_loader:
        ...
        for i in range(6):
            loss_even += (Loss_even(dis_item[i]))
        loss = args.converge_coeff * (loss_gender + loss_income + loss_edu + loss_even) + loss_traj 
    ...
    if np.mean(record_loss)<valid_loss_best:
        ...
        model.save('logs/model_distan_Ten.pkl')

    if epoch%20==0:
        model.save(f'logs/model_distan_epoch{epoch}.pkl')
model.save(f'logs/model_distan_last_Ten.pkl')
'''
    model.eval()
    total_valid_step = 0
    l_sum, n = 0.0, 0
