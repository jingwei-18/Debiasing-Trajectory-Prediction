...
model.load('logs/model_no_last.pkl')
...
for x,y in valid_loader:
    ...
    gender_pred_list.append(np.zeros(y[:,0].shape))
    income_pred_list.append(np.zeros(y[:,1].shape))
    education_pred_list.append(np.zeros(y[:,2].shape))
...
df.to_csv('result_no0.8-0.9.csv', index=False)
